...
import com.gravity.prelement.data.DEPENDENCE_TYPE;
import com.gravity.prelement.data.Frequency;
import com.gravity.prelement.db.DAO;
import com.gravity.scorpio.NormalizedText;
import com.gravity.scorpio.PDGGenerationThread;
...
public class DependenceDistiller {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         final int SIZE_THRESHOLD = cmd.hasOption("s") ? Integer.parseInt(cmd.getOptionValue("s")) : 5;
         final int NUMBER_OF_THREADS = cmd.hasOption("t") ? Integer.parseInt(cmd.getOptionValue("t")) : 1;
         ...
         final PDG[] pdgArray;
         {
            ...
            for (int i = 0; i < pdgGenerationThreads.length; i++) {
               pdgGenerationThreads[i] = new Thread(
                  new PDGGenerationThread(methods, pdgs, cfgNodeFactory, pdgNodeFactory, true, true, true, false, SIZE_THRESHOLD));
         ...
         for (final PDG pdg : pdgArray) {
            ...
            for (final PDGNode<?> fromNode : nodes) {
               ...
               AtomicInteger frequencies = fromNodeFrequencies.get(fromNodeHash);
               if (null == frequencies) {
               ...
               for (final PDGEdge edge : edges) {
                  ...
                  if (edge instanceof PDGControlDependenceEdge) {
                     addToNodeHash(fromNodeHash, toNodeHash, toNodeControlFrequencies);
                  }
                  else if (edge instanceof PDGDataDependenceEdge) {
                     addToNodeHash(fromNodeHash, toNodeHash, toNodeDataFrequencies);
                  }
                  else if (edge instanceof PDGExecutionDependenceEdge) {
                     addToNodeHash(fromNodeHash, toNodeHash, toNodeExecutionFrequencies);
                  }
               }
            }
         }
         ...
         calculateFrequencies(fromNodeFrequencies, toNodeControlFrequencies, texts, frequenciesForControlDependence);
         calculateFrequencies(fromNodeFrequencies, toNodeDataFrequencies, texts, frequenciesForDataDependence);
         calculateFrequencies(fromNodeFrequencies, toNodeExecutionFrequencies, texts, frequenciesForExecutionDependence);
         System.out.print("done: ");
         ...
         registerFrequenciesToDatabase(dao, DEPENDENCE_TYPE.CONTROL, frequenciesForControlDependence);
         registerFrequenciesToDatabase(dao, DEPENDENCE_TYPE.DATA, frequenciesForDataDependence);
         registerFrequenciesToDatabase(dao, DEPENDENCE_TYPE.EXECUTION, frequenciesForExecutionDependence);
         dao.close();
         System.out.print("done: ");
   ...
   private static void addToNodeHash(final int fromNodeHash, final int toNodeHash,
      final ConcurrentMap<Integer, ConcurrentMap<Integer, AtomicInteger>> toNodeFrequencies) {

      ConcurrentMap<Integer, AtomicInteger> toNodeHashes = toNodeFrequencies.get(fromNodeHash);
   ...
   private static String getNormalizedText(final PDGNode<?> node) {
      final NormalizedText fromNodeNormalizedText1 = new NormalizedText(node.core);
      final String fromNodeNormalizedText2 = NormalizedText.normalize(fromNodeNormalizedText1.getText());
      return fromNodeNormalizedText2;
   }

   private static void calculateFrequencies(final ConcurrentMap<Integer, AtomicInteger> fromNodeAllFrequencies,
      final ConcurrentMap<Integer, ConcurrentMap<Integer, AtomicInteger>> toNodeAllFrequencies, final ConcurrentMap<Integer, String> texts,
      final ConcurrentMap<Integer, List<Frequency>> allFrequencies) {

      for (final Entry<Integer, ConcurrentMap<Integer, AtomicInteger>> entry : toNodeAllFrequencies.entrySet()) {
         final int fromNodeHash = entry.getKey();
         final int totalTime = fromNodeAllFrequencies.get(fromNodeHash).get();
         final List<Frequency> frequencies = new ArrayList<Frequency>();
         final ConcurrentMap<Integer, AtomicInteger> toNodeFrequencies = entry.getValue();
         for (final Entry<Integer, AtomicInteger> entry2 : toNodeFrequencies.entrySet()) {
            final int toNodeHash = entry2.getKey();
            final int time = entry2.getValue().get();
            final String normalizedText = texts.get(toNodeHash);
            final Frequency frequency = new Frequency((float) time / (float) totalTime, time, toNodeHash, normalizedText);
   ...
   private static void registerTextsToDatabase(final DAO dao, final ConcurrentMap<Integer, String> texts) {
   ...
   private static void registerFrequenciesToDatabase(final DAO dao, DEPENDENCE_TYPE type,
      final ConcurrentMap<Integer, List<Frequency>> allFrequencies) {

      for (final Entry<Integer, List<Frequency>> entry : allFrequencies.entrySet()) {
   ...
   private static void printFrequencies(final String type, final ConcurrentMap<Integer, String> texts,
      final ConcurrentMap<Integer, List<Frequency>> allFrequencies) {

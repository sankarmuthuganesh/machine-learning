...
public class StatementInfo extends ProgramElementInfo implements BlockInfo {
   ...
   public StatementInfo(final ProgramElementInfo ownerBlock, final CATEGORY category, final int startLine, final int endLine) {
   ...
   public enum CATEGORY {
      ...
      Assert("ASSERT"), Break("BREAK"), Case("CASE"), Catch("CATCH"), Continue("CONTINUE"), Do("DO"), Empty("Empty"), Expression("EXPRESSION"), If(
         "IF"), For("FOR"), Foreach("FOREACH"), Return("RETURN"), SimpleBlock("SimpleBlock"), Synchronized("SYNCHRONIZED"), Switch(
            "SWITCH"), Throw("SWITCH"), Try("TRY"), TypeDeclaration("TYPEDECLARATION"), VariableDeclaration("VARIABLEDECLARATION"), While("WHILE");

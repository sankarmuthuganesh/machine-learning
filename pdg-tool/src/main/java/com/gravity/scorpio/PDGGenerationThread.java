...
import com.gravity.scorpio.pdg.PDGMergedNode;
...
public class PDGGenerationThread implements Runnable {
   ...
   public PDGGenerationThread(final List<MethodInfo> methods, final SortedSet<PDG> pdgs, final CFGNodeFactory cfgNodeFactory,
      final PDGNodeFactory pdgNodeFactory, final boolean useOfControl, final boolean useOfData, final boolean useOfExecution,
   ...
   @Override
   public void run() {
      for (int index = INDEX.getAndIncrement(); index < this.methods.size(); index = INDEX.getAndIncrement()) {
         final MethodInfo method = this.methods.get(index);

         try {

            final PDG pdg = new PDG(method, this.pdgNodeFactory, this.cfgNodeFactory, this.useOfControl, this.useOfData, this.useOfExecution,
            ...
            e.printStackTrace();
            System.err.println("ERROR: failed to process the method " + method.name + " in " + method.path);
         }
      }
   }
}

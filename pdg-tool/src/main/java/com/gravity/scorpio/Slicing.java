...
import com.gravity.scorpio.data.NodePairInfo;
...
public class Slicing {
   ...
   public Slicing(final String pathA, final String pathB, final PDGNode<?> startNodeA, final PDGNode<?> startNodeB,
      final SortedMap<PDGNode<?>, PDGNode<?>[]> mappingPDGNodeToPDGNodes, final SortedMap<PDGEdge, PDGEdge[]> mappingPDGEdgeToPDGEdges,
   ...
   public ClonePairInfo perform() {
      if (null == this.clonepair) {
         ...
         this.clonepair = this.perform(this.startNodeA, this.startNodeB, checkedNodesA, checkedNodesB);
      }
      return this.clonepair;
   }

   private ClonePairInfo perform(final PDGNode<?> nodeA, final PDGNode<?> nodeB, final SortedSet<PDGNode<?>> checkedNodesA,
      ...
      final PDGEdgeComparator comparator = new PDGEdgeComparator(this.mappingPDGEdgeToPDGEdges);
      final SortedSet<PDGEdge> bSortedEdgesA = new TreeSet<PDGEdge>(comparator);
      bSortedEdgesA.addAll(bEdgesA);
      final SortedSet<PDGEdge> bSortedEdgesB = new TreeSet<PDGEdge>(comparator);
      bSortedEdgesB.addAll(bEdgesB);
      final SortedSet<PDGEdge> fSortedEdgesA = new TreeSet<PDGEdge>(comparator);
      fSortedEdgesA.addAll(fEdgesA);
      final SortedSet<PDGEdge> fSortedEdgesB = new TreeSet<PDGEdge>(comparator);
      fSortedEdgesB.addAll(fEdgesB);

      final List<ClonePairInfo> bClonepairs = this.enlargeBackwardClonePair(bSortedEdgesA, bSortedEdgesB, checkedNodesA, checkedNodesB);
      final List<ClonePairInfo> fClonepairs = this.enlargeForwardClonePair(fSortedEdgesA, fSortedEdgesB, checkedNodesA, checkedNodesB);
   ...
   private void makeCandidates(final List<ClonePairInfo> candidates, final List<ClonePairInfo> clonepairs) {
      ...
      for (final ClonePairInfo clonepair : clonepairs) {
         ...
         final ClonePairInfo newCandidate = new ClonePairInfo(this.pathA, this.pathB);
         newCandidate.merge(clonepair);
         candidates.add(newCandidate);
      }
   }

   private List<ClonePairInfo> enlargeBackwardClonePair(final SortedSet<PDGEdge> edgesA, final SortedSet<PDGEdge> edgesB,
      ...
      EDGEA: for (final PDGEdge edgeA : edgesA) {
         ...
         if (checkedNodesA.contains(edgeA.fromNode) || checkedNodesB.contains(edgeA.fromNode)) {
            continue EDGEA;
         }

         final PDGNode<?>[] equivalentNodesA = this.mappingPDGNodeToPDGNodes.get(edgeA.fromNode);
         ...
         EDGEB: for (final PDGEdge edgeB : edgesB) {
            ...
            if (checkedNodesB.contains(edgeB.fromNode) || checkedNodesA.contains(edgeB.fromNode)) {
               continue EDGEB;
            }

            final PDGNode<?>[] equivalentNodesB = this.mappingPDGNodeToPDGNodes.get(edgeB.fromNode);
            ...
            if (equivalentNodesA == equivalentNodesB) {
               ...
               final SortedSet<PDGNode<?>> newCheckedNodesA = new TreeSet<PDGNode<?>>(checkedNodesA);
               final SortedSet<PDGNode<?>> newCheckedNodesB = new TreeSet<PDGNode<?>>(checkedNodesB);
               final ClonePairInfo clonepair = this.perform(edgeA.fromNode, edgeB.fromNode, newCheckedNodesA, newCheckedNodesB);
   ...
   private List<ClonePairInfo> enlargeForwardClonePair(final SortedSet<PDGEdge> edgesA, final SortedSet<PDGEdge> edgesB,
      final Set<PDGNode<?>> checkedNodesA, final Set<PDGNode<?>> checkedNodesB) {
      ...
      EDGEA: for (final PDGEdge edgeA : edgesA) {
         ...
         if (checkedNodesA.contains(edgeA.toNode) || checkedNodesB.contains(edgeA.toNode)) {
            continue EDGEA;
         }

         final PDGNode<?>[] equivalentNodesA = this.mappingPDGNodeToPDGNodes.get(edgeA.toNode);
         if (null == equivalentNodesA) {
            continue EDGEA;
         }
         ...
         EDGEB: for (final PDGEdge edgeB : edgesB) {
            ...
            if (checkedNodesB.contains(edgeB.toNode) || checkedNodesA.contains(edgeB.toNode)) {
               continue EDGEB;
            }

            final PDGNode<?>[] equivalentNodesB = this.mappingPDGNodeToPDGNodes.get(edgeB.toNode);
            if (null == equivalentNodesB) {
               continue EDGEB;
            }
            ...
            if (equivalentNodesA == equivalentNodesB) {
               ...
               final SortedSet<PDGNode<?>> newCheckedNodesA = new TreeSet<PDGNode<?>>(checkedNodesA);
               final SortedSet<PDGNode<?>> newCheckedNodesB = new TreeSet<PDGNode<?>>(checkedNodesB);
               final ClonePairInfo clonepair = this.perform(edgeA.toNode, edgeB.toNode, newCheckedNodesA, newCheckedNodesB);
               clonepairs.add(clonepair);
            }
         }
      }
   ...
   class PDGEdgeComparator implements Comparator<PDGEdge> {
      ...
      PDGEdgeComparator(final SortedMap<PDGEdge, PDGEdge[]> mappingPDGEdgeToPDGEdges) {

...
public class HashCalculationThread implements Runnable {
   ...
   public HashCalculationThread(final PDG[] pdgs, final SortedMap<PDG, SortedMap<PDGNode<?>, Integer>> mappingPDGToPDGNodes,
   ...
   public void run() {
      ...
      for (int index = INDEX.getAndIncrement(); index < this.pdgs.length; index = INDEX.getAndIncrement()) {
         ...
         try {
            ...
            for (final PDGEdge edge : pdg.getAllEdges()) {
               ...
               final String fromNodeText = NormalizedText.normalize(t1.getText());
               final NormalizedText t2 = new NormalizedText(edge.toNode.core);
               final String toNodeText = NormalizedText.normalize(t2.getText());
            ...
            e.printStackTrace();
            System.err.println("ERROR: failed to process the method " + pdg.unit.name + " in " + pdg.unit.path);
         }
      }
   }
}

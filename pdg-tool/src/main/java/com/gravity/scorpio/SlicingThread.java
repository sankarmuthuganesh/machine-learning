...
import com.gravity.scorpio.data.EdgePairInfo;
...
public class SlicingThread implements Runnable {
   ...
   SlicingThread(final PDGPairInfo[] pdgpairs, final PDG[] pdgs, final SortedMap<PDG, SortedMap<PDGNode<?>, Integer>> mapPDGToPDGNodes,
      final SortedMap<PDG, SortedMap<PDGEdge, Integer>> mapPDGToPDGEdges, final SortedSet<ClonePairInfo> clonepairs, final int SIZE_THRESHOLD) {
   ...
   public void run() {
      ...
      for (int index = PAIRINDEX.getAndIncrement(); index < this.pdgpairs.length; index = PAIRINDEX.getAndIncrement()) {
         ...
         try {
            ...
            final SortedMap<PDGNode<?>, Integer> mappingPDGNodeToHashA = this.mapPDGToPDGNodes.get(pdgA);
            final SortedMap<PDGNode<?>, Integer> mappingPDGNodeToHashB = this.mapPDGToPDGNodes.get(pdgB);
            ...
            final SortedMap<PDGEdge, Integer> mappingPDGEdgeToHashA = this.mapPDGToPDGEdges.get(pdgA);
            final SortedMap<PDGEdge, Integer> mappingPDGEdgeToHashB = this.mapPDGToPDGEdges.get(pdgB);
            ...
            final SortedSet<PDGEdge[]> sortedPDGEdges = new TreeSet<PDGEdge[]>(new PDGEdgesComparator());
            for (final List<PDGEdge> list : mappingHashToPDGEdges.values()) {
               if (1 < list.size()) {
                  final PDGEdge[] edges = list.toArray(new PDGEdge[0]);
            ...
            for (final PDGEdge[] edges : sortedPDGEdges) {
               for (int x = 0; x < edges.length; x++) {
                  for (int y = 0; y < edges.length; y++) {
                     ...
                     final NodePairInfo nodepair = new NodePairInfo(edgeA.fromNode, edgeB.fromNode);
                     ...
                     final Slicing slicing = new Slicing(pathA, pathB, edgeA.fromNode, edgeB.fromNode, mappingPDGNodeToPDGNodes,
            ...
            e.printStackTrace();
            System.err.println("ERROR: failed to detect clones between the method " + pdgA.unit.name + " in " + pathA + " and the method"
               + pdgB.unit.name + " in " + pathB);
         }
      }

      for (int index = SINGLEINDEX.getAndIncrement(); index < this.pdgs.length; index = SINGLEINDEX.getAndIncrement()) {
         ...
         try {
            ...
            final SortedMap<PDGNode<?>, Integer> mappingPDGNodeToHash = this.mapPDGToPDGNodes.get(pdg);
            final SortedMap<Integer, List<PDGNode<?>>> mappingHashToPDGNodes = new TreeMap<Integer, List<PDGNode<?>>>();
            ...
            final SortedMap<PDGEdge, Integer> mappingPDGEdgeToHash = this.mapPDGToPDGEdges.get(pdg);
            final SortedMap<Integer, List<PDGEdge>> mappingHashToPDGEdges = new TreeMap<Integer, List<PDGEdge>>();
            ...
            for (final PDGEdge[] edges : sortedPDGEdges) {
               for (int x = 0; x < edges.length; x++) {
                  for (int y = x + 1; y < edges.length; y++) {
                     ...
                     final Slicing slicing = new Slicing(path, path, edgeA.fromNode, edgeB.fromNode, mappingPDGNodeToPDGNodes,
                        mappingPDGEdgeToPDGEdges, checkedNodepairs);
                     final ClonePairInfo clonepair = slicing.perform();
                     if (this.SIZE_THRESHOLD <= clonepair.size()) {
                        clonepairs.add(clonepair);
            ...
            e.printStackTrace();
            System.err.println("ERROR: failed to detect clones in the method " + pdg.unit.name + " in " + path);
         }
      }
   ...
   private void registerNodes(final SortedMap<Integer, List<PDGNode<?>>> mappingHashToPDGNodes,
      ...
      for (final Entry<PDGNode<?>, Integer> entry : mappingPDGNodeToHash.entrySet()) {
   ...
   private void registerEdges(final SortedMap<Integer, List<PDGEdge>> mappingHashToPDGEdges, final SortedMap<PDGEdge, Integer> mappingPDGEdgeToHash) {
      ...
      for (final Entry<PDGEdge, Integer> entry : mappingPDGEdgeToHash.entrySet()) {
         final Integer hash = entry.getValue();
   ...
   private boolean sameOnGoodValue(final ClonePairInfo pair1, final ClonePairInfo pair2, final float threshold) {
      ...
      final float good = Math.min((float) intersectionA.size() / (float) unionA.size(), (float) intersectionB.size() / (float) unionB.size());

      return threshold <= good;
   }

   private boolean sameOnOkValue(final ClonePairInfo pair1, final ClonePairInfo pair2, final float threshold) {
      ...
      final float ok = Math.min(
         Math.max((float) intersectionA.size() / (float) edges1A.size(), (float) intersectionA.size() / (float) edges2A.size()),
         Math.max((float) intersectionB.size() / (float) edges1B.size(), (float) intersectionB.size() / (float) edges2B.size()));

package com.gravity.scorpio.pdg;
...
import com.gravity.pdg.node.PDGNormalNode;
...
public class PDGMergedNode extends PDGNormalNode<ProgramElementInfo> {
   ...
   public static void mergeNodes(final PDG pdg) {
      ...
      MERGE: while (true) {
         ...
         for (final PDGEdge edge : edges) {
            ...
            if (!(fromNode instanceof PDGNormalNode<?>) || !(toNode instanceof PDGNormalNode<?>)) {
               continue;
            }

            final NormalizedText fromNodeText = new NormalizedText(fromNode.core);
            final NormalizedText toNodeText = new NormalizedText(toNode.core);
            if (NormalizedText.normalize(fromNodeText.getText()).equals(NormalizedText.normalize(toNodeText.getText()))) {

               final PDGMergedNode mergedNode = new PDGMergedNode((PDGNormalNode<?>) fromNode, (PDGNormalNode<?>) toNode);
   ...
   private static void replace(final PDGNode<?> replacedNode, final PDGNode<?> replacingNode) {

      assert null != replacedNode : "\"replacedNode\" is null.";
      assert null != replacingNode : "\"replacingNode\" is null.";

      final SortedSet<PDGEdge> backwardEdges = replacedNode.getBackwardEdges();
      for (final PDGEdge backwardEdge : backwardEdges) {
         final boolean b1 = backwardEdge.fromNode.removeForwardEdge(backwardEdge);
         final boolean b2 = replacedNode.removeBackwardEdge(backwardEdge);

         assert b1 : "invalid status.";
         assert b2 : "invalid status.";

         final PDGEdge newBackwardEdge = backwardEdge.replaceToNode(replacingNode);
      ...
      for (final PDGEdge forwardEdge : forwardEdges) {
         final boolean b1 = forwardEdge.toNode.removeBackwardEdge(forwardEdge);
         final boolean b2 = replacedNode.removeForwardEdge(forwardEdge);

         assert b1 : "invalid status.";
         assert b2 : "invalid status.";

         final PDGEdge newForwardEdge = forwardEdge.replaceFromNode(replacingNode);
   ...
   public PDGMergedNode(final PDGNormalNode<?> node1, final PDGNormalNode<?> node2) {
      super(node1.core);
      this.originalNodes = new TreeSet<PDGNormalNode<?>>(new LocationalComparator());
      this.add(node1);
      this.add(node2);
   }

   public SortedSet<PDGNormalNode<?>> getOriginalNodes() {
      final SortedSet<PDGNormalNode<?>> nodes = new TreeSet<PDGNormalNode<?>>(new LocationalComparator());
   ...
   private void add(final PDGNormalNode<?> node) {
      ...
      if (node instanceof PDGMergedNode) {
         final SortedSet<PDGNormalNode<?>> originalNodes = ((PDGMergedNode) node).getOriginalNodes();

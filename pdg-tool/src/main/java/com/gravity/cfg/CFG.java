...
import com.gravity.cfg.edge.CFGControlEdge;
import com.gravity.cfg.edge.CFGEdge;
import com.gravity.cfg.node.CFGBreakStatementNode;
import com.gravity.cfg.node.CFGContinueStatementNode;
import com.gravity.cfg.node.CFGJumpStatementNode;
import com.gravity.cfg.node.CFGNode;
import com.gravity.cfg.node.CFGNodeFactory;
import com.gravity.cfg.node.CFGPseudoNode;
import com.gravity.cfg.node.CFGSwitchCaseNode;
...
public class CFG {
   ...
   private void connectCFGContinueStatementNode(final StatementInfo statement, final CFGNode<? extends ProgramElementInfo> distinationNode) {
      ...
      public void build() {
         ...
         for (int index = 1; index < sequencialCFGs.size(); index++) {
            ...
            for (final CFGNode<?> exitNode : anteriorCFG.exitNodes) {
               final CFGEdge edge = CFGEdge.makeEdge(exitNode, posteriorCFG.enterNode);
               exitNode.addForwardEdge(edge);
               posteriorCFG.enterNode.addBackwardEdge(edge);
            }
         }
   ...
   private final void getReachableNodes(final CFGNode<? extends ProgramElementInfo> startNode,
